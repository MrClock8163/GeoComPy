...
class BEEPINTENSITY(Enum):
    """GSI Online beep signal intensities"""
    OFF = 0
    MEDIUM = 1
    LOUD = 2
...
class ILLUMINATION(Enum):
    """GSI Online illumination settings."""
    OFF = 0
    LEVELONLY = 2
    BOTH = 3
...
class TEMPUNIT(Enum):
    """GSI Online temperature units"""
    CELSIUS = 0
    FAHRENHEIT = 1


class BAUD(Enum):
    """GSI Online serial communication speeds."""
    B1200 = 2
    B2400 = 3
    B4800 = 4
    B9600 = 5
...
class PARITY(Enum):
    """GSI Online serial communication parity."""
    NONE = 0
    ODD = 1
    EVEN = 2
...
class TERMINATOR(Enum):
    """GSI Online serial message terminator."""
    CR = 0
    CRLF = 1
